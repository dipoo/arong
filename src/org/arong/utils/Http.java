...
public class Http {
	...
	protected void storeMessage(){
		try {
			FileUtil.storeStream("/home", "response.txt", receiver);
		} catch (IOException e) {
			e.printStackTrace();
		}
	}
	...
	public synchronized String getHeaderKey(int i) { 
        ...
        Enumeration enum_ = header.propertyNames(); 
        String key = null; 
        for(int j=0; j<=i; j++) 
	...
	public static void main(String[] args)     { 
        Http http=new Http(); 
        http.GET("http://local.gmcc.net:8080/web/gd/customlogin?loginRedirectUrl=/group/gd/workbench");
