...
public class SimpleImage {
     ...
     * @param imageFile
     */
    public SimpleImage(File imageFile) {
        try {
     ...
     * @param imageFilePath
     */
    public SimpleImage(String imageFilePath) {
    ...
    public void combineWithPicture(SimpleImage image2){
        combineWithPicture(image2.getAsBufferedImage(), 2, null);
    }
    public void combineWithPicture(SimpleImage image2, int jump){
            combineWithPicture(image2.getAsBufferedImage(), jump, null);
    }
    
    public void combineWithPicture(SimpleImage image2, Color ignoreColor){
        combineWithPicture(image2.getAsBufferedImage(), 2, ignoreColor);
    }
    public void combineWithPicture(SimpleImage image2, int jump, Color ignoreColor){
    ...
    public static void rebuild(String bgImg, String targetPath, String[] matrix, int xwidth, int yheight, int bgwidth, int bgheight, int x, int y) {
    	try {
    		
			BufferedImage bufferedImagebg = ImageIO.read(new File(bgImg));
			int width = bufferedImagebg.getWidth();
			int height = bufferedImagebg.getHeight();
			
			x = (int) (x * (width * 1.0 / bgwidth));
			y = (int) (y * (height * 1.0 / bgheight));
			double sx = xwidth * Double.parseDouble(matrix[0]) * (width * 1.0 / bgwidth);//x轴缩放
			double sy = yheight * Double.parseDouble(matrix[3]) * (height * 1.0 / bgheight);//y轴缩放
			SimpleImage resizeImage = new SimpleImage(new File(targetPath));
			resizeImage.resize((int)sx, (int)sy);
			BufferedImage bufferedImagetarget = resizeImage.getAsBufferedImage();
			for (int i = 0; i < height; i ++) {
	            for (int j = i % 1; j < width; j +=1) {
	                if(j < x || i < y || j >= x + bufferedImagetarget.getWidth() || i >= y + bufferedImagetarget.getHeight()){
	                    continue; 
	                }else{
	                	try {
		                	int rgb = bufferedImagetarget.getRGB(j - x, i - y);
		                	if(rgb != -1 && bufferedImagebg.getRGB(j, i) != -1){
		                		bufferedImagebg.setRGB(j, i, rgb);
		                	}
		                }catch (Exception e) {
			...
			ImageIO.write(bufferedImagebg, "png", new File(targetPath));
    ...
    public static void rebuild2(String bgImg, String targetPath, String[] matrix, int xwidth, int yheight, int bgwidth, int bgheight, int x, int y) {
    	try {
    		File bgFile = new File(bgImg);
			SimpleImage simpleBgImg = new SimpleImage(bgFile);
			BufferedImage bufferedImagebg = simpleBgImg.getAsBufferedImage();
			
			simpleBgImg.resize(bgwidth, bgheight);
			bufferedImagebg = simpleBgImg.getAsBufferedImage();
			
			SimpleImage simpleBgImg2 = new SimpleImage(bgFile);
			simpleBgImg2.resize(bgwidth, bgheight);
			
			double sx = xwidth * Math.abs(Double.parseDouble(matrix[0]));//x轴缩放
			double sy = yheight * Math.abs(Double.parseDouble(matrix[3]));//y轴缩放
			SimpleImage resizeImage = new SimpleImage(new File(targetPath));
			resizeImage.resize((int)sx, (int)sy);
			BufferedImage bufferedImagetarget = resizeImage.getAsBufferedImage();
			
			//是否旋转
			double degrees = Math.toDegrees(Math.acos(Double.parseDouble(matrix[0])));
			if(degrees != 0 && !Double.isNaN(degrees)){
				if(Double.parseDouble(matrix[0]) < 0){
					degrees = 360 - degrees;
				}else if(Double.parseDouble(matrix[1]) < 0){
					degrees = 360 - degrees;
				}
				
				bufferedImagetarget = RotateImage.rotateImg(bufferedImagetarget, degrees, null);
//				ImageIO.write(bufferedImagetarget, "png", new File(targetPath));
			}
			x += 10;
			y += 10;
			for (int i = 0; i < bgheight; i ++) {
			    for (int j = i % 1; j < bgwidth; j +=1) {
			        if(j < x || i < y || j >= x + bufferedImagetarget.getWidth() || i >= y + bufferedImagetarget.getHeight()){
			             continue;
			        }else{
			        	try {
...
//			ImageIO.write(bufferedImagebg, "png", new File(targetPath));
			...
			simpleBgImg.setBufferedImage(bufferedImagebg);
			simpleBgImg.combineWithPicture(simpleBgImg2, 1, null);
			simpleBgImg.saveAs(targetPath);
			//composeAction(bgImg, targetPath, targetPath, bgwidth, bgheight);
    	} catch (IOException e) {
			e.printStackTrace();
		}
	}
    ...
    public static void composeAction(String bgPath, String targetPath, String finalPath){
    	SimpleImage image  = new SimpleImage(targetPath);
        SimpleImage image2  = new SimpleImage(bgPath);
        image.resize(image2.getWidth(), image2.getHeight());
        image2.resize(image2.getWidth(), image2.getHeight());
        //image.affineTransform(0, 0.3);
        //image2.affineTransform(0, -0.3);
        image.combineWithPicture(image2, 1);
    ...
    public static void main(String[] args) {
        ...
        */
    	...
    	System.out.println(283 * 1.0 / 348);
        
        /*SimpleImage image  = new SimpleImage("e:/icon.jpg");
        SimpleImage image2  = new SimpleImage("e:/choice4.png");
        image.resize(300, 300);
        image2.resize(300, 300);
        image.affineTransform(0, 0.3);
        image2.affineTransform(0, -0.3);
        image.combineWithPicture(image2, 1);
        image.saveAs("e:/out_pic2.png");*/
